...
const SignIn: FC<SignInProps> = () => {
    ...
    return (
        <motion.div
            ...
            <p className='absolute top-2 right-4 z-10  text-gray-800'>Don't have Account ? <span onClick={() => navigate('/signUp')} className='text-violet-800 font-bold cursor-pointer ml-1'> SignUp</span> </p>
            <div className='w-full h-full relative md:bg-transparent bg-white text-gray-900 sm:text-white'>
                ...
                <div className='md:w-[40%] w-full absolute top-20 right-0 px-10 md:px-4 md:mr-2'>
                    ...
                    >
                        ...
                        <Form className='w-full flex  flex-col items-center'>
                            ...
                            <div className=' text-3xl font-bold mb-5 mt-2'>Login</div>
                            <div className='w-full flex justify-center flex-col gap-10'>
                                <div className='w-full'>
                                    <Field className='w-full border-b  outline-none bg-transparent font-medium pb-2 placeholder:text-gray-800 sm:placeholder:text-gray-200' type="email" name='email' id='email' placeholder='Email' />
                                    <span className="text-yellow-400 text-sm"><ErrorMessage name="email" /></span>
                                </div>

                                <div className='w-full relative'>
                                    <Field className='w-full  border-b outline-none bg-transparent font-medium pb-2 placeholder:text-gray-800 sm:placeholder:text-gray-200' type={`${togglePassIcon ? 'password' : 'text'}`} name='password' id='password' placeholder='Password' />
                                    <span onClick={() => setTogglePassIcon(!togglePassIcon)} className='absolute text-xl right-2 cursor-pointer'>{togglePassIcon ? <FaEye /> : <FaEyeSlash />}</span>
                                    <span className="text-yellow-400 text-sm"><ErrorMessage name="password" /></span>
                                </div>
                            </div>
                            <div className='w-full'>
                                <div className=''>
                                    <p className=' mt-4 underline hover:text-blue-900 cursor-pointer'>Forgot Password</p>
                                </div>
                                {isLoading ?
                                    ...
                                    <div className=' w-full h-[2.25rem] mt-7 rounded-md shadow-md'>
                                        <button type='submit' className='bg-green-600  rounded-md w-full h-full font-extrabold'>Login</button>
                                    </div>
                                }
