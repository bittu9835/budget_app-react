...
const BottonNav: FC<BottonNavProps> = () => {
    ...
    return (
        <div className='w-full h-full bg-gray-100 flex items-center justify-between px-5 text-gray-700'>
            <div onClick={()=>navigate('bankAccounts')} className='cursor-pointer text-4xl'><CiBank /></div>
            <div onClick={()=>navigate('cards')} className='cursor-pointer text-4xl'><LiaCreditCardSolid /></div>
            ...
            <div onClick={()=>navigate('settings')} className='cursor-pointer text-4xl'><IoSettingsOutline /></div>
            <div onClick={()=>navigate('transactions')} className='cursor-pointer text-4xl'><AiOutlineFileDone /></div>
        </div>
    );
}
